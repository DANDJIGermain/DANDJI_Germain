...
// get post data
const getPosts = fs.readdirSync(path.join(`content/${blog_folder}`), { recursive: true })
  .map(f => typeof f === 'string' ? f.replace(/\\/g, '/') : f);

const filterPosts = getPosts.filter((post) => {
  const filePath = path.join(`content/${blog_folder}`, post);
  const isFile = fs.lstatSync(filePath).isFile();
  const basename = post.split('/').pop();
  return isFile && basename.match(/^(?!_)/) && post.endsWith(".md");
});
